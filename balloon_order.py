...
def order_balloons(panel, bounded_text):
    ...
    while unordered_balloons:
        ...
        for balloon in unordered_balloons:
            ...
            dist = get_distance(start_x, start_y, balloon_x, balloon_y) + get_distance(
                balloon_x, balloon_y, panel_xmin, panel_ymin
            )
            if dist < min_dist:
                min_dist = dist
                nearest_balloon = balloon
...
if __name__ == "__main__":
    ...
    漫画タイトルの指定
    画像の指定
    画像の指定をしないならif sitei not in img_path: continueをコメントアウト
    吹き出しをmanga109か，画像処理による抽出かを指定
    """
    # パスの設定
    manga109_ano_dir = (
    ...
    for page_index in balloons.keys():
        ...
        # 実験用：img_pathにsiteiが含まれない場合はスキップ
